...
*例程说明  ：1.可以在ecbm_core.h里设置你使用的单片机型号，然后再次编译并下载。
			2.本例程使用了GPIO库、UART库和SOFTSPI库。UART库用于自动下载。SOFTSPI库用于和OLED屏通信。
...
void main(){			//main函数，必须的。
	...
	oled_auto_init(&oled1,"scl=p36,sda=p37,res=p35,dc=p34");//将连接信息输入到初始化函数内.
	//比如现在就是将屏幕的scl脚接到了P36脚，sda接到了P37脚,res脚接到了P35脚，dc脚接到了P34脚。
